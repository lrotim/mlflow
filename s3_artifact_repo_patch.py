...
class S3ArtifactRepository(ArtifactRepository):
    ...
    def delete_artifacts(self, artifact_path=None):
        ...
        for i in range(_MAX_OBJECTS_DELETE_LOOP):
            artifact_response = cli.list_objects(
                Bucket=bucket,
            ...
            if _CONTENTS_STR in artifact_response:
                ...
                if delete_objects:
                    delete_response = cli.delete_objects(
                        Bucket=bucket,
                        Delete={
                            _OBJECTS_STR: delete_objects,
                            _QUIET_STR: True
                        }
                    )
                    if _ERRORS_STR in delete_response:
                        raise ValueError("Error occured while deleting artifacts. Error {}".format(
                            delete_response[_ERRORS_STR]
                        ))
            ...
            if not artifact_response['IsTruncated']:
                ...
                break
        if not over:
            raise ValueError("Failed to fully clean objects at: {}".format(artifact_path))